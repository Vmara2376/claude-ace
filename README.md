...
### v0.8.5（当前版本）
- **P0 安全修复 — GrepTool 命令注入**：改用 `spawnSync` 参数数组传递，彻底消除 shell 注入漏洞。
- **P0 安全修复 — BashTool 过滤加强**：黑名单从 3 条扩充到 20+ 条，新增 pipe-to-shell、系统目录写入、反弹 shell 等危险模式检测。
- **P0 安全修复 — FileWriteTool 路径保护**：路径 `resolve()` 防穿越攻击，阻止写入 `/etc/`、`/bin/`、`C:\Windows\` 等系统目录和敏感文件。
- **P1 修复 — IntentVerify / CriticalArchitect 动态模型**：删除硬编码的 `gpt-4.1-mini`，改为每次请求动态读取用户当前配置的模型和 Key。
- **P1 修复 — Watchdog 防重叠**：加 `_scanning` 标志位 + `try/finally`，防止多次扫描并发执行导致资源竞争。
